...
def tf_format_mnist_images(X, Y, Y_, n=100, lines=10):
    ...
    correctly_recognised_indices = tf.squeeze(
        tf.where(correct_prediction), [1]
    )  # indices of correctly recognised images
    incorrectly_recognised_indices = tf.squeeze(
        tf.where(tf.logical_not(correct_prediction)), [1]
    )  # indices of incorrectly recognised images
    everything_incorrect_first = tf.concat(
        [incorrectly_recognised_indices, correctly_recognised_indices], 0
    )  # images reordered with indeces of unrecognised images first
    everything_incorrect_first = tf.slice(
        everything_incorrect_first, [0], [n]
    ...
    for imslice in range(lines):
        Islices.append(
            tf.concat(
                tf.unstack(
                    tf.slice(
                        I, [imslice * n // lines, 0, 0, 0], [n // lines, 28, 28, 3]
                    )
                ),
                1,
            )
...
class MnistDataVis:
    ...
    def __init__(
        ...
        def _key_event_handler(event):
            ...
            if fignum <= 6:
                ...
                fig.axes[fignum - 1].change_geometry(
                    toggles[keycode][1], toggles[keycode][2], 1
                )
                ax6.set_aspect(25.0 / 40)  # special case for test digits
            elif fignum < 100:
                fig.axes[fignum // 10 - 1].set_visible(True)
                fig.axes[fignum // 10 - 1].change_geometry(
                    toggles[keycode][1], toggles[keycode][2], 1
                )
                fig.axes[fignum % 10 - 1].set_visible(True)
                fig.axes[fignum % 10 - 1].change_geometry(
                    toggles[keycode][1], toggles[keycode][2], 2
                )
    ...
    def append_data_histograms(self, x, datavect1, datavect2, title1=None, title2=None):
        ...
        self.w3 = np.concatenate(
            (self.w3, np.expand_dims(probability_distribution(datavect1), 0))
        )
        datavect2.sort()
        self.b3 = np.concatenate(
            (self.b3, np.expand_dims(probability_distribution(datavect2), 0))
        )
        self._update_xmax(x)
        ...
        def animate_step(i):
            if i == iterations // train_data_update_freq:  # last iteration
                ...
                for k in range(train_data_update_freq):
                    ...
                    request_test_data_update = (n % test_data_update_freq == 0) and (
